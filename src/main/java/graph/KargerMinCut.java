...
public class KargerMinCut {
	...
	public static void main(String[] args) {
		MinCut test = new KargerMinCut().new MinCut();
	}
	...
	class AdjacencyLists {
		...
		public void findEnd(int nodeA, int nodeB) {
			boolean record[] = new boolean[len];// check the node which has been
												// changed yet
		...
		public void addList(int nodeA, int nodeB) {
			while (!nodeHash.get(nodeB).nodeEdge.isEmpty()) {
				nodeHash.get(nodeA).nodeEdge
						.addLast(nodeHash.get(nodeB).nodeEdge.pop());
			}
			// print(nodeA);
		}
	...
	class MinCut {
		private static final String KARGER_MIN_CUT_FILE = "src/main/resources/graph/kargerMinCut.txt";
		...
		public void input() {
			...
			try {
				String file = KARGER_MIN_CUT_FILE;
		...
		public void contract() {
			...
			if (arraySet.size() == 2) {
				...
				// } //test
				System.out
						.println("///" + cutNum + " - " + min + "///" + count); // test
			}
		}

		// computer the edge number of node chosen
		public void tempNum(int a) {
			int num = graph.computerEdge(a); // computer edge of a after
												// contract
			if (num < cutNum) {
				cutNum = num;
			}
		...
		public void randEdge() {
			...
			int randomSeed = graph.nodeHash.get(nodeA).nodeEdge.size();// nodeEdge
																		// index
																		// start
																		// from
																		// 0
